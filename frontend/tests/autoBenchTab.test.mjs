...
test('autobench skeleton uses local draft state so the page can render without backend data', () => {
  ...
  assert.match(autoBenchSource, /const reportSnapshot = ref\(null\)/)
  assert.match(autoBenchSource, /buildAutoBenchReportState/)
  assert.match(autoBenchSource, /const reportState = computed\(/)
...
test('autobench report section renders summary cards failure analysis and export entries without backend wiring', () => {
  assert.match(autoBenchSource, />Generated<\/h3>/)
  assert.match(autoBenchSource, />Summary<\/h3>/)
  assert.match(autoBenchSource, />Failure Analysis<\/h3>/)
  assert.match(autoBenchSource, />Export Entries<\/h3>/)
  assert.match(autoBenchSource, /reportState\.summaryCards/)
  assert.match(autoBenchSource, /reportState\.failureRows/)
  assert.match(autoBenchSource, /reportState\.exportEntries/)
  assert.match(autoBenchSource, /report-entry/)
  assert.doesNotMatch(autoBenchSource, /window\.go|downloadReport|exportReport/)
})
