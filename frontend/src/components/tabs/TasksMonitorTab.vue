<template>
  <div class="tasks-monitor-tab">
    <!-- AutoBench Read-Only Banner -->
    <div v-if="isAutoBenchControlled" class="autobench-readonly-banner">
      <span class="autobench-readonly-icon">&#9888;</span>
      <span>Current task is managed by AutoBench. This page is in read-only observation mode — controls are disabled.</span>
    </div>
...
const canStart = computed(() => !isAutoBenchControlled.value && canPreview.value && !taskBinding.value.startBlocked)
const taskBinding = computed(() => resolveTasksMonitorBinding({
  tasks: taskStore.tasks,
  draft,
  pendingStopTaskId: pendingStopTaskId.value
}))
const canStop = computed(() => !isAutoBenchControlled.value && taskBinding.value.stopEnabled)
